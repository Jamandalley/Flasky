...
class User(UserMixin, db.Model):
    ...
    def confirm(self, token):
        if not current_app.config['SECRET_KEY']:
            raise RuntimeError("SECRET_KEY is not set in app configuration")
        try:
            # Decode and verify the token using the app's secret key
            data = jwt.decode(
                token, 
                current_app.config['SECRET_KEY'], 
                algorithms=['HS256'])
        ...
        # Check if the token's 'confirm' value matches the user's ID
    ...
    def generate_reset_token(self, expiration=3600):
        # Retrieve the secret key from the Flask app configuration
        secret_key = current_app.config['SECRET_KEY']
        # Define the payload for the token containing the user's ID
    ...
    def generate_email_change_token(self, new_email, expiration=3600):
        ...
        try:
            # Encode the payload into a token using the secret key and expiration time
            token_bytes = jwt.encode(payload, secret_key, algorithm='HS256')
            token = token_bytes.decode('utf-8')  # Convert bytes to string
            return token
        except jwt.ExpiredSignatureError:
    ...
    def change_email(token):
        ...
        try:
            ...
            # Handle invalid token error if needed
            return False
        # Extract user_id and new_email from the decoded token data
        user_id = data.get('change_email')
        new_email = data.get('new_email')
        # Check if user_id and new_email are present in the decoded data
        if user_id and new_email:
            # Retrieve the user object from the database based on user_id
            user = User.query.get(user_id)
            if user:
                # Update the user's email address with the new_email
                user.email = new_email
                db.session.commit()  # Commit the database session
                return True
        return False  # Return False if the token is invalid or user not found
    ...
    def verify_auth_token(token):
        ...
        try:
            ...
            # Handle invalid token error if needed
            return None
        # Retrieve the user ID from the decoded token data
        user_id = data.get('id')
        ...
        if user_id is not None:
            ...
            user = User.query.get(user_id)
            return user
        return None  # Return None if the token is invalid or user not found
